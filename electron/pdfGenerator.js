...
export async function generatePOPdf(poData, revisionNumber = 0) {
  try {
    ...
    const baseDir = path.join(app.getPath('documents'), 'UbinkayuERP', 'PO')
    ...
    doc.fontSize(12).text(`Nomor PO   : ${poData.po_number}`)
    ...
    const colWidths = [40, 100, 80, 80, 60, 60, 60] // bisa diatur
    ...
    poData.items.forEach((item, idx) => {
      ...
      row.forEach((val, i) => {
        doc.font('Helvetica').fontSize(9).text(val, x, rowTop, { width: colWidths[i], align: 'center' })
        x += colWidths[i]
      })
      doc.moveDown(1)
    })
    ...
    // --- FOOTER ---
    doc.text('ACC MKT: ...................', { continued: true }).text('  ACC SPV: ...................', { continued: true }).text('  ACC MNGR: ...................')
    doc.moveDown(2)
