...
export async function generatePOJpeg(poData, revisionNumber = 0) {
  try {
    ...
    items.forEach((item) => {
      ...
      ctx.textAlign = 'left'
      wrapText(ctx, poData.po_number || 'N/A', tableLeft + cols.noPo.x + rowPadding, rowTop + rowPadding + 10, cols.noPo.width - rowPadding * 2, lineHeight)
      wrapText(ctx, poData.project_name || 'N/A', tableLeft + cols.noPo.x + rowPadding, rowTop + rowPadding + 25, cols.noPo.width - rowPadding * 2, lineHeight)
      const produkText = `${item.product_name || ''}\n${item.wood_type || ''} ${item.profile || ''}`
      wrapText(ctx, produkText, tableLeft + cols.produk.x + rowPadding, rowTop + rowPadding + 10, cols.produk.width - rowPadding * 2, lineHeight)
      const finishingText = `${item.finishing || ''}\n${item.sample || ''}`
      wrapText(ctx, finishingText, tableLeft + cols.finishing.x + rowPadding, rowTop + rowPadding + 10, cols.finishing.width - rowPadding * 2, lineHeight)
      ctx.textAlign = 'center'
      ...
      ctx.textAlign = 'left'
      wrapText(ctx, item.location || '-', tableLeft + cols.lokasi.x + rowPadding, rowTop + rowPadding + 10, cols.lokasi.width - rowPadding * 2, lineHeight)
      Object.values(cols).forEach((col) => {
    ...
    const notesMarginTop = 20
    rowTop += notesMarginTop
    const notesBoxHeight = 150
    ctx.strokeStyle = borderColor
    ctx.strokeRect(tableLeft, rowTop, tableWidth, notesBoxHeight)
    ctx.fillStyle = blackColor
    ctx.font = 'bold 10px Arial'
    ctx.textAlign = 'left'
    ctx.fillText('Cara kerja / request klien / detail lainnya:', tableLeft + 10, rowTop + 15)
    ctx.font = '10px Arial'
    wrapText(ctx, poData.notes || '-', tableLeft + 10, rowTop + 35, tableWidth - 20, 15)
    rowTop += notesBoxHeight

    const approvalMarginTop = 10
    const approvalBoxWidth = 180
    const approvalBoxHeight = 70
    const approvalX = width - approvalBoxWidth - 30
    const approvalY = rowTop + approvalMarginTop
    ctx.strokeRect(approvalX, approvalY, approvalBoxWidth, approvalBoxHeight)
    ctx.font = 'bold 10px Arial'
    ctx.textAlign = 'center'
    ctx.fillText('ACC MNGR', approvalX + approvalBoxWidth / 2, approvalY + 25)
    ctx.font = '10px Arial'
    ctx.fillText(`Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}`, approvalX + approvalBoxWidth / 2, approvalY + 50)
    rowTop += approvalBoxHeight + approvalMarginTop

    const imageMarginTop = 40
    rowTop += imageMarginTop

    ctx.fillStyle = blackColor
    ctx.font = 'bold 14px Arial'
    ctx.textAlign = 'left'
    ctx.fillText('Lampiran: Foto Referensi (detail - A)', tableLeft, rowTop)
    rowTop += 30
